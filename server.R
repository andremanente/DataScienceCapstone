...
ngrams_accuracy <- readRDS("ngrams_smooth_trim1_prune.rds")
ngrams_speed <- readRDS("ngrams_smooth_trim5_prune.rds")
shortList <- readRDS("shortList.rds")
sampleText <- readRDS("sampleText.rds")
...
cleanTextFull <- function(x) {
    ...
    profanityWords <- readLines(con="profanitywords.txt", skipNul = T)
